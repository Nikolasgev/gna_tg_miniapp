...
async def validate_init_data(
    ...
    После успешной проверки сохраняет покупателя в БД (telegram_id, роль client).
    """
    ...
    if not settings.telegram_bot_token:
        ...
        if settings.is_development:
            ...
            await user_service.upsert_telegram_client(telegram_user.id)
    ...
    if telegram_user.id is not None:
        await user_service.upsert_telegram_client(int(telegram_user.id))
