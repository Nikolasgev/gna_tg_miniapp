...
# Mini App: при наличии backend/static_mini_app раздаём frontend (для ngrok/одного туннеля)
_mini_app_dir = BASE_DIR / "static_mini_app"
if (_mini_app_dir / "index.html").exists():
    app.mount("/", StaticFiles(directory=str(_mini_app_dir), html=True), name="mini_app")
    logger.info(f"Mini App static: {_mini_app_dir}")
else:
