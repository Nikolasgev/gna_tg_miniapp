...
### 4. Быстрый деплой из CLI

Из каталога `backend` (после `railway login` и при необходимости `railway link`):
...
Скрипт выполнит `railway up`. Миграции применяются на стороне Railway через **`preDeployCommand`** в `railway.json` (не с Mac). Другие имена проекта/сервиса:  
`RAILWAY_PROJECT=... RAILWAY_SERVICE=... ./scripts/railway_deploy.sh`
